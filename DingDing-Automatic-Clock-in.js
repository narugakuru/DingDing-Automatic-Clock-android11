...
const CORP_ID = ""
const CORP_NAME = "" //如果获取不了的话可以用这个公司名称
...
function notificationHandler(n) {
    ...
    if (!filterNotification(packageId, abstract, text)) {
        // console.log("通知被过滤: " + packageId + " " + abstract + " " + text)
        return;
    }
    ...
    switch (text) {
            ...
            threads.start(function () {
                switch (DEFAULT_MESSAGE_DELIVER) {
                    ...
                    case PUSH_METHOD.Email:
                        sendQQEmail("考勤结果", getStorageData("dingding", "clockResult"), null)
                        break;
            ...
            threads.start(function () {
                switch (DEFAULT_MESSAGE_DELIVER) {
                    case PUSH_METHOD.QQ:
                        ...
                        break;
                    case PUSH_METHOD.Email:
                        sendQQEmail("修改成功", "已暂停定时打卡功能", null)
                        break;
                    case PUSH_METHOD.ServerChan:
            ...
            threads.start(function () {
                switch (DEFAULT_MESSAGE_DELIVER) {
                    case PUSH_METHOD.QQ:
                        ...
                        break;
                    case PUSH_METHOD.Email:
                        sendQQEmail("修改成功", "已恢复定时打卡功能", null)
                        break;
                    case PUSH_METHOD.ServerChan:
        ...
        case "日志": // 监听文本为 "日志" 的通知
            threads.shutDownAll()
            threads.start(function () {
                let content = files.read(globalLogFilePath);
                sendQQEmail("获取日志", content, null)
            })
            break;
        ...
        threads.start(function () {
            switch (DEFAULT_MESSAGE_DELIVER) {
                case PUSH_METHOD.QQ:
                    ...
                    break;
                case PUSH_METHOD.Email:
                    sendQQEmail("考勤结果", text, cameraFilePath)
                    break;
                case PUSH_METHOD.ServerChan:
 ...
 * @param {string} attachFilePath 要发送的附件路径
 */
function sendQQEmail(title, message, attachFilePath) {
...
function signIn() {
    ...
    // 等待钉钉包名出现，最多等待20秒
    if (!waitForPackage(PACKAGE_ID_DD, 20000)) {
        console.error("等待钉钉启动超时")
    }
...
function attendKaoqin() {
    ...
    // waitForActivity("com.dingtalk.android.activity.AttendanceWebViewActivity");

    waitForPackage("com.alibaba.android.rimet");
    
    // 如果没有CORP_ID，选择指定公司
    if (CORP_ID = "") {
        if (textContains(CORP_NAME).findOne(3000) != null) {
            textContains(CORP_NAME).findOne().click()
            console.log("已选择公司:" + CORP_NAME)
            sleep(1000)
        } else {
            console.log("未找到指定公司:" + CORP_NAME)
        }
    }
...
function clockIn() {
    ...
    if (null != textMatches("上班打卡").clickable(true).findOne(3000)) {
...
function getLastLines(filePath, lineCount) {
    try {
        ...
        // 获取最后lineCount行
        let startIndex = Math.max(0, lines.length - lineCount);
